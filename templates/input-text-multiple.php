	...
	foreach ($input_values as $text_value) {
		...
		<div class="clone-wrapper">
			<input
				...
				?>
			/>
			<!-- Dodajemy klasę, jeśli pole jest disabled, aby łatwiej stylować kursor -->
			<span class="add-field <?php echo $field->is_disabled() ? 'disabled' : ''; ?>" style="<?php echo $field->is_disabled() ? 'cursor: not-allowed; opacity: 0.5;' : 'cursor: pointer;'; ?>">
            <span class="dashicons dashicons-plus-alt"></span>
        </span>
			<span class="remove-field hidden <?php echo $field->is_disabled() ? 'disabled' : ''; ?>" style="<?php echo $field->is_disabled() ? 'cursor: not-allowed; opacity: 0.5;' : 'cursor: pointer;'; ?>">
